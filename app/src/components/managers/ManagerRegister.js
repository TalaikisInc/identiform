...
class ManagerRegister extends PureComponent {
  constructor(props) {
    super(props)
  ...
  getRegFee = async () => {
    this.props.Token.deployed().then(async (token) => {
      token.managerRegistrationFee().then((res) => {
        if (this.mounted) {
          this.setState({
  ...
  getApprovalFee = async () => {
    this.props.Token.deployed().then(async (token) => {
      token.managerApprovalFee().then((res) => {
        if (this.mounted) {
          this.setState({
                ...
                if (error) {
                  ...
                  this.props.ipfs.dag.put(_hash, { format: 'dag-cbor', hashAlg: 'sha2-256' }, async (err, cid) => {
                    if (err) {
                      ...
                      const _gas = await token.newManager.estimateGas('EMPTY', _cidBase, { from: this.props.account, value: _fee })
                      token.newManager('EMPTY', _cidBase, {
        ...
        <Box align='center'>
          { this.props.registeredAsManager && this.props.registered ? done : <div>
            ...
            { !this.props.registered ? <Box align='center'>
              ...
              : <Box align='center'>
                <Label>This form takes ethers as payment, for tokens based form, please <Link to='/manager_tokens'>go here</Link></Label>
                <Form onSubmit={this.handleSubmit}>
                  ...
                  <SelectInput req={true} id='companyCountry' autocomplete='country-name' companyCountry={this.state.companyCountry}
